...
REPORT = ROOT / "VERSION_32_FIELD_LOCATION_MOBILE_REPORT.json"
...
(() => {
  ...
  try {
    ...
    const openPassed = checks.dialogOpen && checks.buttonCount === 9 && checks.positionsComplete && checks.touchTargets && checks.noHorizontalOverflow && checks.dialogInsideViewport && checks.fieldVisible && checks.fieldContainsButtons && checks.formScrollableWhenNeeded && checks.titleVisible;
    if (mode === 'flow') {
...
def run_viewport(cdp, viewport, mode="open", screenshot=True):
    ...
    if screenshot:
        ...
        file = PREVIEW / f"version32-field-location-{slug(viewport['name'])}.png"
...
def main():
    ...
    try:
        ...
        report = {
            ...
            "scope": "Batted-ball position popup and save flow across representative iPhone and iPad portrait/landscape viewports",
            "passed": bool(passed),
            "viewports": results,
